package com.smartwallet.withdrawalservice.client;
...
public class TransactionClient {
    ...
    @Recover
    public TransactionResponse recoverRecordTransaction(
            RestClientException ex,
            CreateTransactionRequest request
    ) {
        log.error(
                "transaction-service unreachable after retries — txId={} type={} amount={}. Manual reconciliation required.",
                request.transactionId(), request.type(), request.amount(), ex
        );
        return null;
    }
}
